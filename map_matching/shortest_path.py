...
def _generate_path(source_node, target_node, scanned_nodes):
    path = []
    node = target_node
    while node is not None:
        edge = scanned_nodes[node]
...
# Deprecated because of inefficiency
def single_source(source_node, get_edges, get_cost_sofar=None):
    scanned_nodes = {}
    pqueue = []
    # Start with a dummy edge
    dummy_edge = Edge(start_node=None, end_node=source_node, cost=0)
    cost_sofar = get_cost_sofar(dummy_edge, 0) if get_cost_sofar else dummy_edge.cost
    if cost_sofar >= 0:
        heapq.heappush(pqueue, (cost_sofar, dummy_edge))
    ...
    while pqueue:
        ...
        cur_node = edge.end_node
        scanned_nodes[cur_node] = edge
        yield cur_node, scanned_nodes
        for adj_edge in get_edges(cur_node):
            if adj_edge.end_node in scanned_nodes:
                continue
            if get_cost_sofar:
                adj_cost_sofar = get_cost_sofar(adj_edge, cost_sofar)
            else:
                adj_cost_sofar = cost_sofar + adj_edge.cost
            if adj_cost_sofar >= 0:
                heapq.heappush(pqueue, (adj_cost_sofar, adj_edge))
...
# Deprecated because of inefficiency
def one_to_many(source_node, target_nodes, get_edges, get_cost_sofar=None):
    goal_set = set(target_nodes)
    generator = single_source(source_node, get_edges, get_cost_sofar)
    for node, scanned_nodes in generator:
        if node in goal_set:
            paths.append(_reconstruct_path(source_node, node, scanned_nodes))
            goal_set.remove(node)
        if not goal_set:
            break
    return paths
...
# Deprecated because of inefficiency
def one_to_one(source_node, target_node, get_edges, get_cost_sofar=None):
    generator = single_source(source_node, get_edges, get_cost_sofar)
    for node, scanned_nodes in generator:
        if node == target_node:
            generator.close()
            return _reconstruct_path(source_node, node, scanned_nodes)
    raise PathNotFound(source_node, target_node)


# Deprecated because of inefficiency
def astar(source_node, target_node, get_edges, get_distance):
    def get_cost_sofar(node, costsofar):
        return costsofar + node.cost + get_distance(node, target_node)
    return one_to_one(source_node, target_node, get_edges, get_cost_sofar)
...
def test_one_to_one():
    ...
    # It should find correct path
    _, cost = one_to_one(1, 5, _get_edges)
    assert cost == 20
    path, cost = one_to_one(1, 1, _get_edges)
    assert path == []
    assert cost == 0
    _, cost = one_to_one(1, 5, _get_edges)
    assert cost == 20
    _, cost = one_to_one(1, 4, _get_edges)
    assert cost == 20

    # It should not find a path
    def get_cost_sofar(node, prev_cost_sofar):
        cost_sofar = node.cost + prev_cost_sofar
        return -1 if cost_sofar > 19 else cost_sofar

    from nose.tools import assert_raises
    assert_raises(PathNotFound, one_to_one, 1, 4, _get_edges, get_cost_sofar)
    assert_raises(PathNotFound, one_to_one, 1, 'no such node', _get_edges)
    assert_raises(PathNotFound, one_to_one, 'no such node', 1, _get_edges)
